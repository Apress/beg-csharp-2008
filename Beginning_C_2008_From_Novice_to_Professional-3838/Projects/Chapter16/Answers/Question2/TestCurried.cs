...
namespace FunctionalProgramming {
    ...
    class TestCurried {
        ...
        static void ExampleSalesSalesTaxCalculator() {
            Func<double[], Func<double, double>> salesTax = 
                localSalesTax => totalBought => {
                    double runningSalesTax = 0.0;
                    foreach( double tax in localSalesTax) {
                        runningSalesTax += (totalBought * tax);
                    }
                    return totalBought + runningSalesTax;
                };

            Func<double, double> localplace = salesTax(new double[] { 0.07, 0.08 });
            double amount = 100.0;
        ...
        public static void RunAll() {
            ...
            ExampleSalesSalesTaxCalculator();
        }
    }
}
